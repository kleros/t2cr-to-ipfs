 ...
 * @param invalidTokens These are tokens that failed 0xsequence-tokenlists validation rules.
 */
export function getNewNFTListVersion(
  previousTokens: CollectibleList,
  latestTokens: CollectibleInfo[],
  invalidTokens: CollectibleInfo[],
): Version {
  ...
  for (const latestToken of latestTokens) {
    ...
    if (!previousToken) {
      if (invalidTokens.includes(latestToken)) {
